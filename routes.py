...
def generate_report():
    ...
    if report_format == 'csv':
        si = io.StringIO()
        cw = csv.writer(si)
        cw.writerow(['ID', 'Asset ID', 'Severity', 'CVE', 'Description', 'Recommendation', 'Created At'])
        for f in findings:
            cw.writerow([f.id, f.asset_id, f.severity, f.cve, f.description, f.recommendation, f.created_at])
